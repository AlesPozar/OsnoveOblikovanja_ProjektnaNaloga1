...
import { Skin, getAllWeaponTypes, skinToColor, dominantMasksSkins, averageMasksSkins, weightedMasksSkins } from "@/lib/skinData";
...
export default function ColorSection({
  ...
  // Choose masks dataset corresponding to this section index
  const masksForSection = useMemo(() => {
    if (sectionIndex === 1) return dominantMasksSkins;
    if (sectionIndex === 2) return averageMasksSkins;
    if (sectionIndex === 3) return weightedMasksSkins;
    return skins;
  }, [sectionIndex]);

  // If either the local toggle or the global analysis mode requests ingame, show masks
  const effectiveIngame = ingameMode || analysisMode === "ingame";
  const coneSkins = effectiveIngame ? masksForSection : skins;
  ...
  return (
    ...
    >
      ...
      <header className="relative flex items-center justify-between px-10 py-4 flex-shrink-0">
        ...
        >
          ...
          <button
            ...
            className={`flex flex-col items-center transition-colors ${ingameMode ? "text-foreground/80" : "text-foreground/30"
              ...
              <div className="absolute right-0 top-full mt-1 bg-white border border-gray-100 rounded shadow-xl z-50 max-h-64 overflow-y-auto w-56">
                ...
                {selectedNames.map((name) => {
                  ...
                  return (
                    <button
                      key={name}
                      className={`block w-full text-left px-3 py-1.5 text-sm font-light hover:bg-gray-50 transition-colors ${highlightId === s?.id
      ...
      <div className="flex flex-1 min-h-0 relative">
        ...
        >
          <div className={`${panelVisible ? "-translate-x-16" : ""} flex flex-col items-center`}>
            <HSVCone
              skins={coneSkins}
            ...
            <div className="flex items-center gap-6 mt-2">
              ...
              <button
                ...
                className={`transition-opacity ${view === "cone" ? "opacity-100" : "opacity-25 hover:opacity-50"
                  }`}
              >
                <svg
                  viewBox="0 0 28 28"
              ...
              <button
                ...
                className={`transition-opacity ${view === "circle" ? "opacity-100" : "opacity-25 hover:opacity-50"
                  }`}
              >
                <svg
                  viewBox="0 0 28 28"
